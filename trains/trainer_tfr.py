...
from data_create import create_cifar_data
from models import name_map_models
from optimizer_tool import get_optimizer

map_function = create_cifar_data.dataset_parse_function
...
class Trainer(object):
    ...
    def build_for_train(self):
        ...
        with tf.Graph().as_default(), tf.device('/cpu:0'):
            with tf.variable_scope(tf.get_variable_scope()):
                ...
                train_set = train_set.map(map_function)
                ...
                test_set = test_set.map(map_function)
                ...
                next_img, next_label, _ = iterator.get_next()

                # ****** 数据归一化 *************
                image = tf.reshape(next_img, [tf.shape(next_img)[0], train_config.img_height,
                                              train_config.img_width, train_config.img_ch], 'image')
                ...
                norm_image = (float_image - train_config.img_mean) / train_config.img_var
                # ******************************

                image_splits = tf.split(norm_image, num_gpus)
                ...
                for d in self.gpu_id:
                    with tf.device('/gpu:%s' % d):
                        with tf.name_scope('%s_%s' % ('tower', d)):
                            loss, acc = \
                                self.model.optimize_model(image_splits[counter], label_splits[counter])
                            counter += 1
            ...
            mean_acc = tf.stack(axis=0, values=tower_acc)
            mean_acc = tf.reduce_mean(mean_acc, 0)
    ...
    def train(self):
        ...
        error_summary = tf.summary.scalar('acc_rate', self.layers['acc'])
        ...
        train_summaries = tf.summary.merge(summaries)  # error_summary
        ...
        for epoch in range(self.total_epochs):
            ...
            _, step, loss, acc, lr_rate, train_merged = sess.run([self.layers['optimizer'],
            ...
            if step % self.disp_inter == 0:
                log_str = '[%s] %.3f sec/batch \t step:%d\t lr:%.6f \t loss:%.6f \t acc rate:%.6f' % (
                    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    (end_time - start_time), step, lr_rate, loss, acc)
            ...
            if step % self.test_inter == 0:
                ...
                self.average_loss = 0.0
                average_acc_rate = 0.0
                batch_num = 0
                while True:
                    try:
                        test_acc_rate, test_loss = sess.run([
                            self.layers['acc'],
                            self.layers['loss']],
                            feed_dict={self.layers['handle']: validation_handle})
                        self.average_loss += test_loss
                        average_acc_rate += test_acc_rate
                        batch_num += 1
                    except tf.errors.OutOfRangeError:
                        break

                self.average_loss = self.average_loss / batch_num
                average_acc_rate = average_acc_rate / batch_num
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_inter', type=int, default=100, help='save interval(batch num)')
    parser.add_argument('--test_inter', type=int, default=100, help='test interval(batch num)')
    parser.add_argument('--disp_inter', type=int, default=100, help='display in summary interval(batch num)')
    ...
    parser.add_argument('--model_type', type=str, default='crnn', help='model type, current support two:crnn, cnn')
    parser.add_argument('--lr_type', type=str, default='naive', help='learning rate adjust strategy:naive,sgdr')

    parser.add_argument('--need_padded_patch', type=int, default=0, help='need padded batch:0,1')
